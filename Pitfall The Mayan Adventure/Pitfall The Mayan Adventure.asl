...
start
{
    return (old.start == 0x00 && current.start == 0xFF || old.start == 0x00 && current.start == 0xFF && current.choose == 0x30);
}
split
{
    ...
    if (old.level == 0x68 && current.level == 0x6C) return true;//Tikal Ruins
...
startup
{
	...
	settings.Add("Runaway_Minecar", true, "Runaway Minecar", "options");
}